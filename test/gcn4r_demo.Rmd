...
set.seed(42)
ego.net<-graph_from_adjacency_matrix(ego.nets[[23]])
...
Adj<-Adj/n.sim
Adj
plot.net(asIgraph(simulate(mod)))
net.list.2<-net.list
...
Adj<-as.matrix(Adj)
diag(Adj)<-1.
Adj.gnn<-simulate.networks(link.model,30)$Adj
Adj.gnn2<-matrix(0,nrow=N,ncol=N)
for (i in 1:n.sim){
  Adj.gnn2<-Adj.gnn2+Adj.gnn[[i]]
}
Adj.gnn2<-Adj.gnn2/30
Adj.true<-as.matrix(as_adjacency_matrix(extract.graphs(link.model)$A.true))
diag(Adj.true)<-1
sklearn<-import("sklearn")
set.seed(42)
sklearn$metrics$roc_auc_score(as.vector(Adj.true),as.vector(Adj))
sklearn$metrics$roc_auc_score(as.vector(Adj.true),as.vector(as.matrix(as_adjacency_matrix(asIgraph(simulate(mod))))))
sklearn$metrics$roc_auc_score(as.vector(Adj.true),as.vector(Adj.gnn2))

opt.threshold<-optimize(function(x) -sklearn$metrics$f1_score(as.vector(Adj.true),as.vector(Adj>x)),c(0,1))$minimum
sklearn$metrics$f1_score(as.vector(Adj.true),as.vector(Adj>opt.threshold))
...
vis.weighted.graph(abs(stdinf),weight.scaling.factor = 0.7,cl=extract.clusters(cluster.model),threshold=1.96)
