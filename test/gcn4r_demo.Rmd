...
net.list.2$A<-(net.list.2$A+t(net.list.2$A))/2
diag(net.list.2$A)<-0
prediction.col<-which(colnames(net.list.2$X)=="practice2")-1
...
torch<-import("torch")
for (i in 1:4){
  vis.weighted.graph(attn.matrices[[i]],cl=torch$argmax(prediction.model$results$y,dim=1L)$numpy(),threshold=0.05)
}

vis.weighted.graph(attn.matrices[[2]]%*%attn.matrices[[3]],cl=torch$argmax(prediction.model$results$y,dim=1L)$numpy(),threshold=0.06)

vis.weighted.graph(attn.matrices[[1]]%*%attn.matrices[[2]]%*%attn.matrices[[3]]%*%attn.matrices[[4]],cl=torch$argmax(prediction.model$results$y,dim=1L)$numpy(),threshold=0.02,weight.scaling.factor = 0.75)
